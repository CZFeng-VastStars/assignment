...
class ModelNetDataset():
    def __init__(self, root, batch_size=32, npoints=1024, split='train', normalize=True, normal_channel=False,
        ...
        if modelnet10:
            self.catfile = os.path.join(self.root, 'modelnet_shape_names.txt')
        else:
        ...
        shape_ids = {}
        if modelnet10:
            shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.root, 'modelnet_train.txt'))]
            shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.root, 'modelnet_test.txt'))]
        else:
            shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.root, 'modelnet_train.txt'))]
            shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.root, 'modelnet_test.txt'))]
...
if __name__ == '__main__':
    d = ModelNetDataset(root='./data/modelnet', split='test')
    print(d.shuffle)
    print(len(d))
    import time
